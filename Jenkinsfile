pipeline {
    ...
    stages {
        ...
        stage('Start Grid') {
            steps {
                sh "THREAD_COUNT=${params.THREAD_COUNT} TEST_SUITE=${params.TEST_SUITE} FOLDER=(${params.TEST_SUITE//./ }) docker compose -f docker-grid.yaml up --scale ${params.BROWSER}=${params.NUMBER_OF_BROWSERS} -d"
            }
        }
